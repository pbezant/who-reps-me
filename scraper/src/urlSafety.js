...
// than one an LLM recalled (discover.js) or a human hand-authored (config/seeds.json): the
// "suggest an official" submission form (netlify/functions/submit-official.mjs). Everywhere
// else in the scraper, the fetched URL either came from our own LLM call or from a file only
// a contributor can edit — this is the only path where the URL itself is untrusted input, so
// it's the only place that needs an SSRF guard at all.
//
